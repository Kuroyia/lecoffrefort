...
function isSolved() {
    ...
    const currentOrder = Array.from(tiles).map(tile => tile.src.slice(-5, -4));
    const correctOrder = ["1", "2", "3", "4", "5", "6", "7", "8", "9"];
    return JSON.stringify(currentOrder) === JSON.stringify(correctOrder);
}

function displayWinMessage() {
    // Display your win message, e.g., show an alert
    $('.special-text').show();
}

function hideBoard() {
    // Hide the board, you can set display: none or any other method you prefer
